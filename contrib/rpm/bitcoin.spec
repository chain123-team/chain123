...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11222
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 10222
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 111222
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 110222
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 11222
	%{_sbindir}/semanage port -d -p tcp 10222
	%{_sbindir}/semanage port -d -p tcp 111222
	%{_sbindir}/semanage port -d -p tcp 110222
	for selinuxvariant in %{selinux_variants}; do
