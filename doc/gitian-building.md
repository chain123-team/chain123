...
*Setup instructions for a Gitian build of Chain123 Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Chain123
...
- [Building Chain123 Core](#building-chain123-core)
...
Clone the git repositories for chain123 and Gitian.
...
in the chain123 repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Chain123 Core
----------------

To build Chain123 Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the chain123 repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/chain123/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/chain123-project/chain123
...
For example:
```bash
URL=https://github.com/thrasher-/chain123.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit chain123=${COMMIT} --url chain123=${URL} ../chain123/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit chain123=${COMMIT} --url chain123=${URL} ../chain123/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit chain123=${COMMIT} --url chain123=${URL} ../chain123/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the chain123 git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../chain123/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/chain123
SIGPATH=/some/root/path/chain123-detached-sigs

./bin/gbuild --url chain123=${BTCPATH},signature=${SIGPATH} ../chain123/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/chain123-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/chain123-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/chain123-osx-build.assert
```
...
[chain123-project/gitian.sigs.c123](https://github.com/chain123-project/gitian.sigs.c123/) repository, or if that's not possible create a pull
