...
Below are some notes on how to build Chain123 Core for Windows.

The options known to work for building Chain123 Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Chain123 Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Chain123 Core applications.
...
Note that for WSL the Chain123 Core source path MUST be somewhere in the default mount file system, for
example /usr/src/chain123, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/chain123-project/chain123.git
    sudo chmod -R a+rw chain123
...
way. This will install to `c:\workspace\chain123`, for example:

    make install DESTDIR=/mnt/c/workspace/chain123
...
causes two of the chain123 executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the chain123 code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the chain123 source code).
