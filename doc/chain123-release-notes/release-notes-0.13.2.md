Chain123 Core version 0.13.2 is now available from:

  <https://download.chain123.org/chain123-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/chain123-project/chain123/issues>
...
updates will be released anymore. Without security updates, using a chain123
...
ECDSA signatures inside Chain123 transactions now use validation using
...
Previous versions of Chain123 Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Chain123 Core was to raise the default minimum
relay fee.

Chain123 Core 0.13.2 will have a strict maximum size on the mempool. The
...
Chain123 Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running chain123d without having to do any manual configuration.
...
Chain123 Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Chain123 Core will relay transactions with
...
entirely in Chain123 Core 0.13, and it is at this time undecided whether the
...
Chain123 Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Chain123 Core automatically creates a hidden service to listen on, without
manual configuration. Chain123 Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Chain123 Core is listening, and
...
Chain123d can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Chain123
Core to use floating fees. By default, Chain123 Core will use floating
fees.
...
(default: `0.0002` C123/kB).

At all times, Chain123 Core will cap fees at `-maxtxfee=<x>` (default:
0.10) C123.
Furthermore, Chain123 Core will never create transactions paying less than
...
`chain123.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 211222 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 211222 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /chain123rpc>
        ProxyPass http://127.0.0.1:11222/
        ProxyPassReverse http://127.0.0.1:11222/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass chain123d rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< chain123rpc:<password>
    </Location>
...
Restarting chain123d will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in chain123.conf
...
chain123-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/chain123-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Chain123 Core code base has
...
- `chain123-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `chain123-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Chain123 scripting system that in combination with
...
Chain123 transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Chain123 Core.
...
The two phases are distinct in the Chain123-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Chain123 wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Chain123.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Chain123 block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of chain123s they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of chain123s being spent is exactly
...
- **Increased security for multisig:** Chain123 addresses (both P2PKH addresses
  ...
  believed to be provided by Chain123's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Chain123's current ledger (called the UTXO set),
  ...
  note that Chain123 Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Chain123 users to individually opt-in to almost any change in the Chain123
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Chain123 Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Chain123
client to Chain123 Core 0.13.2 to signal for segwit support. When a
...
Since Chain123 Core 0.10.0, nodes have defaulted to only relaying and
...
- `chain123-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `chain123-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
